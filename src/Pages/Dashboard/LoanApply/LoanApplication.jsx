...
const LoanApplication = () => {
  ...
  return (
    <div className="overflow-x-auto">
      <p className="text-xl  font-bold text-[#ff8000] py-3">
      ...
      <table className="w-full text-sm text-center text-gray-500 dark:text-gray-400">
        ...
        {loanApplied?.length === 0 ? (
          <caption className="flex my-5 mx-auto justify-center font-medium">
            <span>Loan Request Not Available</span>{" "}
          </caption>
        ) : (
          <>
            {" "}
            {loanApplied.map((loan, index) => (
              <tbody key={index}>
                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                  ...
                  <td className="px-6">
                    {" "}
                    <span className="text-xl">৳</span> {loan?.LoanAmount}
                  </td>
                  <td className="px-6">
                    {loan?.loan === "accepted" ? (
                      <span className="text-green-600 font-bold mx-2 text-center text-sm px-2 py-1">
                        Accepted
                      </span>
                    ) : loan?.loan === "rejected" ? (
                      <span className="text-red-600 font-bold mx-2 text-center text-sm px-2 py-1">
                        Rejected
                      </span>
                    ) : (
                      <>
                  ...
                  <td>
                    {loan && (
                      <button
                        ...
                        className=" text-white bg-gradient-to-r from-green-400 via-green-500 font-semibold to-green-600 hover:bg-gradient-to-br  text-xs px-2 py-1 rounded"
                      >
                        Application Details
                      </button>
                    )}
